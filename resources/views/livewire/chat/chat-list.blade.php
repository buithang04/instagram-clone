...
class="flex flex-col transition-all h-full overflow-hidden">
    ...
    <header class="px-3 z-10 bg-white sticky top-0 w-full py-2 sm:pt-12">
        ...
        {{-- Filters --}}
        <section class="  gap-3 grid grid-cols-3 items-center mt-1  p-2 bg-white">
        ...
        @if ($conversations != null)
            ...
            <ul class="p-2 grid w-full spacey-y-2">
                ...
                @foreach ( $conversations as $conversation )
                    ...
                    {{-- Chat list item --}}
                    <li  class="py-3   {{request()?->chat == $conversation?->id ? 'bg-gray-300':'' }}  hover:bg-gray-50 rounded-2xl dark:hover:bg-gray-700/70 transition-colors duration-150 flex gap-4 relative w-full cursor-pointer px-2">
                        
                        <a href="{{route('profile.home',$receiver->username)}}" class="shrink-0">
                            <x-avatar wire:ignore  src="https://i.pravatar.cc/500?img={{rand(0,10)}}"  class="w-12 h-12" />
                        </a>
