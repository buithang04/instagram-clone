<div x-data="{
    ...
    }" class="menu p-3  w-20  h-full grid  border-r  text-black" :class="{'w-72 ':!shrink}">
    ...
    <ul class="space-y-4 mt-2">
        ...
        <li><div @click="showSearch=false;showNotifications=true;drawer=true;"
        ...
        @auth
            <li>
                <a wire:navigate href="{{ route('profile.home', auth()->user()->username)}}"
                    class="flex items-center gap-5">
                    <x-avatar class="w-7 h-7 shrink-0" src="https://i.pravatar.cc/400?img=" />
                    <h4 x-cloak x-show="!(shrink || drawer)"
    ...
    <footer class="sticky bottom-0 mt-auto w-full grid px-3 z-50 ">
        <div class="dropdown dropdown-top ">
            <label tabindex="0" class=" cursor-pointer   flex items-center w-full gap-5 m-1">
                <span>
            ...
            <ul tabindex="0" class="dropdown-content z-[1] menu p-2 space-y-3 shadow   rounded-box w-60">
                ...
                <li>
                    <a href="#" class="py-2 flex items-center gap-5"
    ...
    <div x-show="drawer" x-cloak x-transition.origin.left
        @click.outside="showSearch=false;showNotifications=false;drawer=false;"
        class="fixed inset-y-0 left-[70px] w-96 px-4 overflow-y-scroll  shadow bg-white border rounded-r-2xl z-[50]">
        ...
        <template x-if="showSearch">
            <div class="h-full">
                ...
                <main>
                    @if ($results)
                        <ul class="spacy-y-2 overflow-x-hidden">
                        @foreach ($results as $key => $user )
                            
                        
                        <li>
                            <a href="{{route('profile.home',$user->username)}}" class=" flex truncate gap-2 items-center">
                                <x-avatar wire:ignore class="w-9 h-9 mb-auto" src="https://i.pravatar.cc/500?img-{{ $key }}"
                                />
