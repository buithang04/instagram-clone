...
<div class="grid lg:grid-cols-12 gap-3 h-full w-full overflow-hidden">
    ...
    <aside class="hidden  lg:flex lg:col-span-7 m-auto items-center w-full">
    {{-- Container cho carousel --}}
    <div class="relative flex overflow-x-scroll w-[500px]  snap-x snap-mandatory gap-2 px-2 ">
        ...
        @foreach ($post->media as $key => $file)
            ...
            <div class="w-full h-full shrink-0 snap-center">
                @switch($file->mime)
                    @case('video')
                        <x-video source="{{$file->url}}" class="w-full h-full object-scale-down"/>
                        @break
    ...
    <aside class=" lg:col-span-5 h-full scrollbar-hide relative flex gap-4 flex-col overflow-hidden overflow-y-scroll ">
        <header class="flex items-center gap-3 border-b py-2 sticky top-0 bg-white z-10">
            <x-avatar wire:ignore story src="https://i.pravatar.cc/150?img={{rand(1,10)}}" class="h-9 w-9" />
        ...
        class="grid grid-cols-12 items-center w-full">
            ...
            <button class="col-span-1 ml-auto">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
